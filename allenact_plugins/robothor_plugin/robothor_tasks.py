...
class ObjectNavTask(Task[RoboThorEnvironment]):
    ...
    def judge(self) -> float:
        """Judge the last event."""
        reward = self.reward_configs["step_penalty"]
        # distance = self.env.distance_to_object_type(
        #     self.task_info["object_type"]
        # )
        #r2
        reward += self.shaping()
        ...
        #     last = self.path[-2]
        #     d = math.sqrt(
        #                     (now["x"] - last["x"]) ** 2 + (now["z"] - last["z"]) ** 2
        #                     )
        #     if d > 0.2 :
        #         reward -= 0.5*self.reward_configs["step_penalty"]
        #r1
        # # print(len(self.path))
        # if distance < 4:
        #     reward += self.shaping()
        ...
        #         is_new = True
        #         now = self.path[-1]
        #         for id,p in enumerate(self.path[:-1]):
        #             if id in ids:
        #                 now_topast = math.sqrt(
        #                 (now["x"] - p["x"]) ** 2 + (now["z"] - p["z"]) ** 2
        #                 )
        #                 if now_topast<1:
        #                     is_new = False
        #                     break
        #         if is_new:
        #             ids.append(len(self.path) - 2)
        #             reward -= 25*self.reward_configs["step_penalty"]
        # print(reward)
