...
class Action_boost_model(ActorCriticModel[CategoricalDistr]):
    def __init__(
        self,
        action_space: gym.spaces.Discrete,
        observation_space: SpaceDict,
        ...
        self.actor = LinearActorHead(self._hidden_size, action_space.n)
        self.lastx = []
        self.actor2 = LinearActorHead(self._hidden_size, action_space.n)
        self.actor3 = LinearActorHead(self._hidden_size, action_space.n)
        self.actor4 = LinearActorHead(self._hidden_size, action_space.n)
        self.actor5 = LinearActorHead(self._hidden_size, action_space.n)

        self.actors=[self.actor2,self.actor3,self.actor4,self.actor5]
        self.critic = LinearCriticHead(self._hidden_size)
        if self.include_auxiliary_head:
            self.auxiliary_actor = LinearActorHead(self._hidden_size, action_space.n)
    ...
    def forward(  # type:ignore
        ...
        if len(self.lastx)>0:
            action = a1
            for i,xi in enumerate(reversed(self.lastx)):
                actor = self.actors[i]
                ...
                if last.shape == action.shape:
                    #for test
                    #action = torch.add(action,(1.1-(i+1)*0.2)*last)
                    action = torch.add(action,last)
        else:
            action = a1
        self.lastx.append(x.detach())
        if len(self.lastx) == 5:
            self.lastx=self.lastx[1:]
        return (
            ActorCriticOutput(
                distributions=CategoricalDistr(logits=action),
                values=self.critic(x),
                extras={"auxiliary_distributions": self.auxiliary_actor(x)}
                if self.include_auxiliary_head
                else {},
